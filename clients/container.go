...
func JFDockerDelete(args string) (*apis.RspJFDocker,error) {
    ...
    if status == "ok" { Logger("Successful to delete the app container",delData.Appname) }
...
func JFDockerRebuilt(args string) error {
    ...
    if err != nil { log.Fatalf("Failed to get the runConf for container"+rebuiltname,err.Error()) }
    ...
    if result,err := JFDockerDelete(rebuiltname);err == nil {
        ...
        } else { log.Fatalf("Failed to run container",rebuiltname) }         

    } else { log.Fatalf("Failed to delete container",rebuiltname) }
    return nil
}
